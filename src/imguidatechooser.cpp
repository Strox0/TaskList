...
namespace ImGui {
    ...
    IMGUI_API bool TimeChooserPopup(const char* label, tm& timeOut, bool* pSetStartTimeToTimeOutThisFrame, const char* upArrow, const char* downArrow)
    {
        ...
        static tm t = GetDateZero();

        if (pSetStartTimeToTimeOutThisFrame && *pSetStartTimeToTimeOutThisFrame) 
        {
			*pSetStartTimeToTimeOutThisFrame = false;
			t = timeOut;
		}
		else if (timeOut.tm_hour == 0 && timeOut.tm_min == 0) 
        {
            t = GetCurrentDate();
            timeOut.tm_hour = t.tm_hour;
            timeOut.tm_min = t.tm_min;
		}
		...
		const char* arrowUp = upArrow ? upArrow : "^";
		const char* arrowDown = downArrow ? downArrow : "v";
		const ImVec2 arrowUpsize = ImGui::CalcTextSize(arrowUp);
		const ImVec2 arrowDownsize = ImGui::CalcTextSize(arrowDown);
        const float buttonWidth = ImGui::CalcTextSize("00").x + style.FramePadding.x * 2;

        const float width = buttonWidth*2 + style.ItemSpacing.x*2 + ImGui::CalcTextSize(":").x + 10;
        const float height = ImGui::CalcTextSize("00").y + style.FramePadding.y *2 + arrowDownsize.y + arrowUpsize.y + 10 + style.ItemSpacing.y*2;

        ImGui::SetNextWindowSize(ImVec2(width, height));

        bool value_changed = false;

        ImGui::PushStyleVar(ImGuiStyleVar_WindowPadding, style.ItemSpacing);
        ...
        if (ImGui::BeginPopup(label, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse))
        {
            ...
            ImGui::PushID(1234);
            if (ImGui::Button(arrowUp, { buttonWidth , arrowUpsize.y }))
            {
                t.tm_hour += 1;
				if (t.tm_hour > 23) 
					t.tm_hour = 0;
            ...
            ImGui::SameLine();
            ImGui::SetCursorPosX(width - style.ItemSpacing.x - buttonWidth);
            ImGui::PopID();
            if (ImGui::Button(arrowUp, { buttonWidth , arrowUpsize.y }))
            {
                t.tm_min += 1;
            ...
            ImGui::PushStyleVar(ImGuiStyleVar_FramePadding, { 0,0 });
            ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(1, 1, 1, 0));
            if (ImGui::Button(arrowDown, { buttonWidth , arrowDownsize.y }))
            {
                t.tm_hour -= 1;
                if (t.tm_hour < 0) 
                    t.tm_hour = 23;
...
#   endif //IMGUIVARIOUSCONTROLS_H_
    ...
    bool DateChooserPopup(const char* label, tm& dateOut, bool* pSetStartDateToDateOutThisFrame, const char* leftArrow, const char* rightArrow) 
    {
        ImGuiContext& g = *GImGui;
        ImGuiWindow* window = GetCurrentWindow();
        if (window->SkipItems)
        ...
        static const char* dayNames[7] = { "M","T","W","T","F","S","S" };
        ...
        const float width = 2.f * arrowLeftWidth + 2.f * arrowRightWidth + ImGui::CalcTextSize(monthNames[8]).x + ImGui::CalcTextSize("9999").x + ImGui::CalcTextSize("W").x * 7.f;
        const float height = ImGui::CalcTextSize("M").y * 8.f + style.ItemSpacing.y * 8.f + style.FramePadding.y*2;
        ...
        if (ImGui::BeginPopup(label, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse))
        {
            ...
            static char yearString[12] = ""; sprintf(yearString, "%d", 1900 + d.tm_year);
            const float yearPartWidth = arrowLeftWidth + arrowRightWidth + ImGui::CalcTextSize(yearString).x + style.FramePadding.x*5 + style.ItemSpacing.x * 2;
            ...
            ImGui::SameLine();
            ImGui::SetCursorPosX(ImGui::GetWindowWidth() - yearPartWidth);
            ImGui::PushID(1235);
            if (ImGui::SmallButton(arrowLeft)) { d.tm_year -= 1; if (d.tm_year < 0) d.tm_year = 0; RecalculateDateDependentFields(d); }
            ImGui::SameLine();
            ...
            const static int numDaysPerMonth[12] = { 31,28,31,30,31,30,31,31,30,31,30,31 };
            int maxDayOfCurMonth = numDaysPerMonth[d.tm_mon];
            if (maxDayOfCurMonth == 28) 
            {
                const int year = d.tm_year + 1900;
                const bool bis = ((year % 4) == 0) && ((year % 100) != 0 || (year % 400) == 0);
            ...
            for (int dw = 0; dw < 7; dw++) {
                ...
                if (dw == 5 || dw == 6) ImGui::PopStyleColor();
                ImGui::EndGroup();
                if (dw != 6) ImGui::SameLine(ImGui::GetWindowWidth() - (6 - dw) * (ImGui::GetWindowWidth() / 7.f));
            }
